  ...
  return (
    <div style={{
      ...
      <div style={{
        ...
        <div style={{ fontSize: 10, opacity: 0.5, textAlign: "left", marginTop: 5, fontWeight: 400 }}>
          {msg.time}
        </div>
      </div>
      {isUser && (
...
export default function CapiAPI() {
 ...
 const sendMessage = useCallback(async (text = input) => {
    ...
    setMessages(prev => [...prev, { role: "user", content: trimmed, time: now, id: userId }]);
    setInput(""); setLoading(true); setError(null);
    try {
      const aiId = userId + 1;
      const aiTime = new Date().toLocaleTimeString("es-MX", { hour: "2-digit", minute: "2-digit" });
      setMessages(prev => [...prev, { role: "assistant", content: "", time: aiTime, id: aiId }]);
  ...
  }, [input, loading, messages, selectedCourse, selectedCourseCode]);
